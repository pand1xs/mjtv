const { name } = require("../Events/audit_log_create");

module.exports = {
  data: { name: "Set Time Restriction", user: {type: "any", value: ""} },
  category: "Control",
  aliases: ["command cooldown", "timeout command", "event cooldown", "timeout event", "timed repeat", "anti spam", "spam protection", "wait before running", "repeat protection"],
  UI: [
    {
      element: "dropdown",
      storeAs: "timeUnit",
    ...
    {
      element: "case",
      name: "If The Time Restriction Is Not Ongoing",
  ...
  async run(values, message, client, bridge) {
    ...
    async function runNotRestricted() {
      if (values.ifNotOngoing) {
