...
module.exports = {
    ...
    subtitle: (data, constants) => {return `List [${constants.variable(data.list)}] convert to text - Store Result As ${constants.variable(data.store)}`},
    compatibility: ["Any"],
          ...
          for (const dataCase of values.cases) {
            ...
            if (posicao == (i + irseguir)) {
              ...
              if (matchesCriteria == true) {
                ...
                const start3 = bridge.transf(dataCase.data.start3).replace("\\n", "\n");
                const end3 = bridge.transf(dataCase.data.end3).replace("\\n", "\n");
